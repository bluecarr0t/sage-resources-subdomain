#!/usr/bin/env npx tsx
/**
 * Publish Spring Creek Ranch and split inn / townhome / villa inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-spring-creek-ranch-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-spring-creek-ranch-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
       ...
       FROM all_sage_data
       WHERE property_id = 'e9d6dce9-8eca-4c48-a22b-2c688e8a7939'
       ORDER BY
         CASE unit_type
           WHEN 'Hotel Room' THEN 1
           WHEN 'Suite' THEN 2
           WHEN 'Villa' THEN 3
           ELSE 4
         END,
    ...
    console.log('✓ Spring Creek Ranch enrichment applied');
    console.table(rows);
