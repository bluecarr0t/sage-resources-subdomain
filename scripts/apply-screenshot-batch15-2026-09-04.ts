#!/usr/bin/env npx tsx
/**
 * Reject the 17 in-progress screenshot properties (2026-09-04 batch 15).
 * Also rejects the Enchanted Earth Retreat Playa sibling (same property_id).
 *
 * Usage:
 *   npx tsx scripts/apply-screenshot-batch15-2026-09-04.ts
 *
...
const MIGRATIONS = [
  {
    name: 'Reject screenshot batch15 ghosts',
    file: 'reject-screenshot-batch15-ghosts-2026-09-04.sql',
    where: `id IN (174, 168, 161, 69, 71, 173, 75, 151, 124, 11061, 150, 154, 172, 134, 135, 176, 137, 119)`,
  },
] as const;
  ...
  try {
    ...
    for (const m of MIGRATIONS) {
      ...
      if (rejected.length !== rows.length) {
        throw new Error(
          `expected all batch15 rows rejected; got published=${published.length}`
        );
      }
    }
    ...
    const { rows: untouched } = await client.query(
      `SELECT id, property_name, city, country, research_status
       FROM all_sage_data
       WHERE id IN (41, 42, 57, 10723, 10942, 13074)
       ORDER BY id`
    );
    console.log(
      'Expect untouched (Outaouais Treehouses / Refuges Perchés / San Cristóbal / Wildpod / Owl\'s Perch / Northeast Cove):'
    );
