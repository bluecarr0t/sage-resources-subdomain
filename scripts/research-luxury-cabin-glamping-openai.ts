#!/usr/bin/env npx tsx
/**
 * Research glamping properties with LUXURY CABIN unit type, minimum 5 total units.
 * North America (USA, Canada, Mexico). Fuzzy dedup on property_name, address, city, state.
 * Inserts with research_status = 'new'.
 *
 * Usage:
 *   npx tsx scripts/research-luxury-cabin-glamping-openai.ts
 *   npx tsx scripts/research-luxury-cabin-glamping-openai.ts --dry-run
 *   npx tsx scripts/research-luxury-cabin-glamping-openai.ts --limit 10
 */
...
function buildResearchQueries(limit?: number): ResearchQuery[] {
  ...
  const unitRequirement = `CRITICAL: Include ONLY properties that have at least ${MIN_UNITS} luxury cabin units (quantity_of_units or property_total_sites >= ${MIN_UNITS}). Exclude single-cabin or 1-4 unit properties.`;

  return [
    {
      label: 'USA - Luxury Cabin Resorts (5+ units)',
      country: 'USA',
      prompt: `You are a researcher for an outdoor hospitality directory. List LUXURY CABIN glamping resorts in the United States.

${unitRequirement}

Include properties that:
- Offer luxury cabins as a primary accommodation type (high-end cabins, upscale cottages, premium lodges)
- Have at least 5 luxury cabin units on site
- Are currently operating
- Focus on glamping-style luxury cabins (not basic vacation rentals or traditional hotels)
- May use terms: luxury cabin, upscale cabin, boutique cabin, designer cabin, glamping cabin
- Exclude standard campground cabins and budget motels

${limitClause}

For each property provide: property_name (required), city (required), state (required), address, zip_code, url (official website), phone_number, description (2-4 sentences emphasizing luxury cabin experience), property_type (e.g. "Glamping Resort", "Luxury Cabin Resort"), unit_type (must include "Luxury Cabin" or "luxury cabins"), quantity_of_units (required - number of cabin units, must be >= 5), property_total_sites (if different), lat, lon if known.

Return a JSON object with a "properties" array. Only include properties with 5+ luxury cabin units.`,
    },
    {
      label: 'Canada - Luxury Cabin Resorts (5+ units)',
      country: 'Canada',
      prompt: `You are a researcher for an outdoor hospitality directory. List LUXURY CABIN glamping resorts in Canada.

${unitRequirement}

Include properties across provinces (BC, Ontario, Quebec, etc.) that:
- Offer luxury cabins as a primary unit type
- Have at least 5 luxury cabin units on site
- Are currently operating
- Focus on glamping-style luxury cabins

${limitClause}

For each property provide: property_name (required), city (required), state (province), address, zip_code (postal code), url (official website), phone_number, description (2-4 sentences), property_type, unit_type (must include "Luxury Cabin"), quantity_of_units (required, >= 5), property_total_sites, lat, lon if known. Use country: "Canada".

Return a JSON object with a "properties" array. Only include properties with 5+ luxury cabin units.`,
    },
    {
      label: 'Mexico - Luxury Cabin Resorts (5+ units)',
      country: 'Mexico',
      prompt: `You are a researcher for an outdoor hospitality directory. List LUXURY CABIN glamping resorts in Mexico.

${unitRequirement}

Include properties in Tulum, Riviera Maya, Oaxaca, and other regions that:
- Offer luxury cabins (cabañas de lujo) as a primary unit type
- Have at least 5 luxury cabin units on site
- Are currently operating
- Focus on glamping-style luxury cabins
...
For each property provide: property_name (required), city (required), state (region), address, zip_code, url (official website), phone_number, description (2-4 sentences), property_type, unit_type (must include "Luxury Cabin"), quantity_of_units (required, >= 5), property_total_sites, lat, lon if known. Use country: "Mexico".
...
function toDbRow(p: DiscoveredProperty): Record<string, unknown> {
  ...
  return {
    ...
    is_open: 'Yes',
    source: 'Sage',
    discovery_source: 'OpenAI Research - Luxury Cabins',
    date_added: TODAY,
    date_updated: TODAY,
...
async function main() {
  ...
  console.log(`Researching LUXURY CABIN properties (min ${MIN_UNITS} units) in North America...\n`);
  ...
  console.log(`\nTotal unique new luxury cabin properties to add: ${uniqueNew.length}`);
  ...
  console.log(`\nDone. Added ${uniqueNew.length} luxury cabin properties (min ${MIN_UNITS} units) with research_status='new'.`);
}
