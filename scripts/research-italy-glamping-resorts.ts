#!/usr/bin/env npx tsx
/**
 * Research glamping resorts in Italy using OpenAI API and save to CSV
 * 
 * This script:
 * - Uses OpenAI API to research glamping resorts in Italy
 ...
 * 
 * Usage:
 *   npx tsx scripts/research-italy-glamping-resorts.ts
 */
...
const CSV_FILE = 'csv/glamping-properties/europe/italy-glamping-resorts.csv';

// Search queries to find glamping resorts in Italy
const SEARCH_QUERIES = [
  'best glamping resorts Italy 2024',
  'luxury glamping Italia',
  'glamping sites Tuscany Italy',
  'glamping sites Italian Riviera Italy',
  'glamping sites Lake Como Italy',
  'glamping sites Dolomites Italy',
  'glamping sites Amalfi Coast Italy',
  'glamping sites Sardinia Italy',
  'glamping sites Sicily Italy',
  'glamping sites Umbria Italy',
  'treehouse glamping Italy',
  'yurt glamping Italy',
  'safari tent glamping Italy',
  'luxury camping Italy',
  'glamping retreats Italy',
  'eco glamping Italy',
  'glamping near Rome Italy',
  'glamping near Milan Italy',
];
...
/**
 * Research glamping properties in Italy using OpenAI
 */
async function researchGlampingProperties(searchQuery: string): Promise<PropertyData[]> {
  console.log(`🔍 Researching: "${searchQuery}"\n`);

  const prompt = `Research glamping resorts and retreats in Italy based on this search query: "${searchQuery}"

Find glamping properties (luxury camping resorts, tent accommodations, yurts, treehouses, safari tents, etc.) located in Italy.
...
- state (optional): Italian region name (e.g., "Tuscany", "Lombardy", "Sardinia", "Sicily")
- country: "Italy"
...
- phone_number (optional): Phone number if available (with country code +39)
...
async function enrichPropertyData(property: PropertyData): Promise<PropertyData> {
  ...
  const prompt = `Research and provide comprehensive detailed information about this glamping property in Italy:
...
Return a JSON object with these fields:
{
  ...
  "city": "City name",
  "state": "Italian region name (e.g., Tuscany, Lombardy, Sardinia, Sicily)",
  "country": "Italy",
  "address": "Full street address if available",
  "zip_code": "Postal code if available",
  "url": "Official website URL",
  "phone_number": "Phone number with country code (+39)",
...
Focus on Italian glamping properties. If you cannot find specific information, use null or omit the field. Be thorough and accurate.
...
async function main() {
  ...
  console.log('Research Glamping Resorts in Italy');
  console.log('='.repeat(70));
