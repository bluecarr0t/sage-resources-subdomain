#!/usr/bin/env npx tsx
/**
 * Research glamping properties with HOBBIT HOME unit type, minimum 5 total units.
 * North America (USA, Canada, Mexico). Fuzzy dedup on property_name, address, city, state.
 * Inserts with research_status = 'new'.
 *
 * Usage:
 *   npx tsx scripts/research-hobbit-home-glamping-openai.ts
 *   npx tsx scripts/research-hobbit-home-glamping-openai.ts --dry-run
 *   npx tsx scripts/research-hobbit-home-glamping-openai.ts --limit 10
 */
...
function buildResearchQueries(limit?: number): ResearchQuery[] {
  ...
  const unitRequirement = `CRITICAL: Include ONLY properties that have at least ${MIN_UNITS} hobbit home units (quantity_of_units or property_total_sites >= ${MIN_UNITS}). Exclude single-unit or 1-4 unit properties.`;

  return [
    {
      label: 'USA - Hobbit Home Resorts (5+ units)',
      country: 'USA',
      prompt: `You are a researcher for an outdoor hospitality directory. List HOBBIT HOME glamping resorts in the United States.

${unitRequirement}

Include properties that:
- Offer hobbit homes (earth-sheltered, underground, or Lord of the Rings-style dwellings) as a primary unit type
- Have at least 5 hobbit home units on site
- Are currently operating
- May use terms like: hobbit holes, hobbit houses, underground homes, earth homes, cave dwellings, fantasy cottages
- May also offer other units, but hobbit homes must be a main offering

${limitClause}

For each property provide: property_name (required), city (required), state (required), address, zip_code, url (official website), phone_number, description (2-4 sentences emphasizing hobbit/underground experience), property_type (e.g. "Glamping Resort", "Hobbit Resort"), unit_type (must include "hobbit home" or "hobbit homes"), quantity_of_units (required - number of hobbit units, must be >= 5), property_total_sites (if different), lat, lon if known.

Return a JSON object with a "properties" array. Only include properties with 5+ hobbit home units.`,
    },
    {
      label: 'Canada - Hobbit Home Resorts (5+ units)',
      country: 'Canada',
      prompt: `You are a researcher for an outdoor hospitality directory. List HOBBIT HOME glamping resorts in Canada.

${unitRequirement}

Include properties across provinces (BC, Ontario, Quebec, etc.) that:
- Offer hobbit homes or earth-sheltered dwellings as a primary unit type
- Have at least 5 hobbit home units on site
- Are currently operating

${limitClause}

For each property provide: property_name (required), city (required), state (province), address, zip_code (postal code), url (official website), phone_number, description (2-4 sentences), property_type, unit_type (must include "hobbit home"), quantity_of_units (required, >= 5), property_total_sites, lat, lon if known. Use country: "Canada".

Return a JSON object with a "properties" array. Only include properties with 5+ hobbit home units.`,
    },
    {
      label: 'Mexico - Hobbit Home Resorts (5+ units)',
      country: 'Mexico',
      prompt: `You are a researcher for an outdoor hospitality directory. List HOBBIT HOME glamping resorts in Mexico.

${unitRequirement}

Include properties in Tulum, Riviera Maya, Oaxaca, and other regions that:
- Offer hobbit homes or earth-sheltered dwellings (casas hobbit, cuevas) as a primary unit type
- Have at least 5 hobbit home units on site
- Are currently operating

${limitClause}

For each property provide: property_name (required), city (required), state (region), address, zip_code, url (official website), phone_number, description (2-4 sentences), property_type, unit_type (must include "hobbit home"), quantity_of_units (required, >= 5), property_total_sites, lat, lon if known. Use country: "Mexico".
...
function toDbRow(p: DiscoveredProperty): Record<string, unknown> {
  ...
  return {
    ...
    is_closed: 'No',
    source: 'Sage',
    discovery_source: 'OpenAI Research - Hobbit Homes',
    date_added: TODAY,
    date_updated: TODAY,
...
async function main() {
  ...
  console.log(`Researching HOBBIT HOME properties (min ${MIN_UNITS} units) in North America...\n`);
  ...
  console.log(`\nTotal unique new hobbit home properties to add: ${uniqueNew.length}`);
  ...
  console.log(`\nDone. Added ${uniqueNew.length} hobbit home properties (min ${MIN_UNITS} units) with research_status='new'.`);
}
