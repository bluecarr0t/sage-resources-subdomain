#!/usr/bin/env npx tsx
/**
 * Research glamping properties with SAFARI TENT unit type, minimum 5 total units.
 * North America (USA, Canada, Mexico). Fuzzy dedup on property_name, address, city, state.
 * Inserts with research_status = 'new'.
 *
 * Usage:
 *   npx tsx scripts/research-safari-tent-glamping-openai.ts
 *   npx tsx scripts/research-safari-tent-glamping-openai.ts --dry-run
 *   npx tsx scripts/research-safari-tent-glamping-openai.ts --limit 10
 */
...
function buildResearchQueries(limit?: number): ResearchQuery[] {
  ...
  const unitRequirement = `CRITICAL: Include ONLY properties that have at least ${MIN_UNITS} safari tent units (quantity_of_units or property_total_sites >= ${MIN_UNITS}). Exclude single-tent or 1-4 unit properties.`;

  return [
    {
      label: 'USA - Safari Tent Resorts (5+ units)',
      country: 'USA',
      prompt: `You are a researcher for an outdoor hospitality directory. List SAFARI TENT glamping resorts in the United States.

${unitRequirement}

Include properties that:
- Offer safari tents as a primary unit type (safari tents, luxury tents, glamping tents)
- Have at least 5 safari tent units on site
- Are currently operating
- May also offer cabins or other units, but safari tents must be a main offering
- Exclude basic camping tents or traditional campgrounds
...
Return a JSON object with a "properties" array. Only include properties with 5+ safari tent units.`,
    ...
    {
      label: 'Canada - Safari Tent Resorts (5+ units)',
      country: 'Canada',
      prompt: `You are a researcher for an outdoor hospitality directory. List SAFARI TENT glamping resorts in Canada.

${unitRequirement}

Include properties across provinces (BC, Ontario, Quebec, etc.) that:
- Offer safari tents as a primary unit type
- Have at least 5 safari tent units on site
- Are currently operating
...
Return a JSON object with a "properties" array. Only include properties with 5+ safari tent units.`,
    ...
    {
      label: 'Mexico - Safari Tent Resorts (5+ units)',
      country: 'Mexico',
      prompt: `You are a researcher for an outdoor hospitality directory. List SAFARI TENT glamping resorts in Mexico.

${unitRequirement}

Include properties in Tulum, Riviera Maya, Oaxaca, and other regions that:
- Offer safari tents as a primary unit type (tiendas safari, glamping tents)
- Have at least 5 safari tent units on site
- Are currently operating
...
function toDbRow(p: DiscoveredProperty): Record<string, unknown> {
  ...
  return {
    ...
    is_closed: 'No',
    source: 'Sage',
    discovery_source: 'OpenAI Research - Safari Tents',
    date_added: TODAY,
    date_updated: TODAY,
...
async function main() {
  ...
  console.log(`Researching SAFARI TENT properties (min ${MIN_UNITS} units) in North America...\n`);
  ...
  console.log(`\nTotal unique new safari tent properties to add: ${uniqueNew.length}`);
  ...
  console.log(`\nDone. Added ${uniqueNew.length} safari tent properties (min ${MIN_UNITS} units) with research_status='new'.`);
}
