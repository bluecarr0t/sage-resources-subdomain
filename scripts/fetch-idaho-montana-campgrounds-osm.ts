#!/usr/bin/env npx tsx
/**
 * Fetch campgrounds from Idaho and Montana using OpenStreetMap Overpass API
 * 
 * This script:
 * - Queries OpenStreetMap Overpass API for all campgrounds in Idaho and Montana
 ...
 * 
 * Usage:
 *   npx tsx scripts/fetch-idaho-montana-campgrounds-osm.ts
 */
...
const STATE_REGIONS: StateRegion[] = [
  // Idaho - split into regions to avoid timeout
  ...
  // Montana - split into regions to avoid timeout
...
function extractCampgroundData(feature: OSMFeature, state: string): {
  ...
  if (operator) {
    ...
    // State indicators
             ...
             (state === 'ID' && operatorLower.includes('idaho')) ||
             (state === 'MT' && operatorLower.includes('montana'))) {
...
async function main() {
  ...
  console.log('Fetch Idaho and Montana Campgrounds from OSM');
  console.log('='.repeat(70));
  console.log();
