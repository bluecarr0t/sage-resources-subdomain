...
For each property provide: property_name (required), city (required), state (required), address, zip_code, url (official website), phone_number, description (2-4 sentences), property_type (e.g. "Glamping Resort", "Marina with Glamping", "Outdoor Resort", "Landscape Hotel"), unit_type (single singular Title Case label preferred — e.g. "Bell Tent", "Yurt", "Cabin", "Safari Tent", "Cabin Tent"; ambiguous "tents"/"glamping tents" → omit, not Safari Tent), lat, lon if known.
