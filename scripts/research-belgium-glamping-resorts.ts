#!/usr/bin/env npx tsx
/**
 * Research glamping resorts in Belgium using OpenAI API and save to CSV
 * 
 * This script:
 * - Uses OpenAI API to research glamping resorts in Belgium
 ...
 * 
 * Usage:
 *   npx tsx scripts/research-belgium-glamping-resorts.ts
 */
...
const CSV_FILE = 'csv/glamping-properties/europe/belgium-glamping-resorts.csv';

// Search queries to find glamping resorts in Belgium
const SEARCH_QUERIES = [
  'best glamping resorts Belgium 2024',
  'luxury glamping België',
  'glamping sites Ardennes Belgium',
  'glamping sites Flanders Belgium',
  'glamping sites Wallonia Belgium',
  'glamping sites Belgian Coast Belgium',
  'glamping sites Antwerp Belgium',
  'glamping sites Ghent Belgium',
  'glamping sites Bruges Belgium',
  'treehouse glamping Belgium',
  'yurt glamping Belgium',
  'safari tent glamping Belgium',
  'luxury camping Belgium',
  'glamping retreats Belgium',
  'eco glamping Belgium',
  'glamping near Brussels Belgium',
  'glamping near Antwerp Belgium',
  'glamping near Ghent Belgium',
];
...
/**
 * Research glamping properties in Belgium using OpenAI
 */
async function researchGlampingProperties(searchQuery: string): Promise<PropertyData[]> {
  console.log(`🔍 Researching: "${searchQuery}"\n`);

  const prompt = `Research glamping resorts and retreats in Belgium based on this search query: "${searchQuery}"

Find glamping properties (luxury camping resorts, tent accommodations, yurts, treehouses, safari tents, etc.) located in Belgium.
...
- state (optional): Belgian region/province name (e.g., "Flanders", "Wallonia", "Brussels", "Antwerp", "West Flanders")
- country: "Belgium"
...
- phone_number (optional): Phone number if available (with country code +32)
  ...
  try {
    ...
    properties = properties
      ...
      .map(p => ({
        ...
        country: p.country?.trim() || 'Belgium',
      }));
...
async function enrichPropertyData(property: PropertyData): Promise<PropertyData> {
  ...
  const prompt = `Research and provide comprehensive detailed information about this glamping property in Belgium:

Property Name: ${property.property_name}
City: ${property.city || 'Unknown'}
State: ${property.state || 'Unknown'}
Country: ${property.country || 'Belgium'}
...
Return a JSON object with these fields:
{
  ...
  "city": "City name",
  "state": "Belgian region/province name (e.g., Flanders, Wallonia, Brussels, Antwerp, West Flanders)",
  "country": "Belgium",
  "address": "Full street address if available",
  "zip_code": "Postal code if available",
  "url": "Official website URL",
  "phone_number": "Phone number with country code (+32)",
...
Focus on Belgian glamping properties. If you cannot find specific information, use null or omit the field. Be thorough and accurate.
...
async function main() {
  ...
  console.log('Research Glamping Resorts in Belgium');
  console.log('='.repeat(70));
