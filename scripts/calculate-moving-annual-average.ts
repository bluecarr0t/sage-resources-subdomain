/**
 * Script to calculate and populate moving-annual-average for county-gdp table
 * 
 * This script:
 * - Fetches all records from county-gdp
 * - Calculates year-over-year percentage changes for each consecutive year pair
 * - Averages all the percentage changes to get the moving annual average
 * - Updates each record with the calculated value
 * 
 * Usage:
 *   npx tsx scripts/calculate-moving-annual-average.ts
 */
...
// Years available in the data (note: 2011 is missing)
const YEARS = [2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023];
...
/**
 * Calculate moving annual average for a single record
 * This is the average of all year-over-year percentage changes
 */
function calculateMovingAnnualAverage(record: CountyGDPRecord): number | null {
  const changes: number[] = [];
  
  // Calculate YoY changes for consecutive years
...
async function updateRecords(
  ...
  updates: Array<{ geofips: string; 'moving-annual-average': number | null }>
): Promise<void> {
  ...
  for (let i = 0; i < updates.length; i += BATCH_SIZE) {
    ...
    const updatePromises = batch.map(async (update) => {
      const { error } = await supabase
        .from(TABLE_NAME)
        .update({ 'moving-annual-average': update['moving-annual-average'] })
...
async function main() {
  try {
    ...
    // Calculate moving annual average for each record
    console.log('\n🧮 Calculating moving annual averages...');
    const updates: Array<{ geofips: string; 'moving-annual-average': number | null }> = [];
    
    let calculated = 0;
    let nullCount = 0;
    
    for (const record of records) {
      const average = calculateMovingAnnualAverage(record);
      updates.push({
        geofips: record.geofips,
        'moving-annual-average': average,
      });
    ...
    console.log(`✅ Calculated averages for ${records.length} records:`);
    ...
    console.log(`  2. Check the moving-annual-average column values`);
