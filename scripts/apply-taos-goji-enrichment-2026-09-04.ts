#!/usr/bin/env npx tsx
/**
 * Publish Taos Goji and split into historic cabin + tipi inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-taos-goji-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(process.cwd(), 'scripts/migrations/enrich-taos-goji-2026-09-04.sql'),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              unit_pets, unit_full_kitchen, property_hot_tub, property_sauna,
              rate_basis, rate_avg_retail_daily_rate, rate_fall_weekday
       FROM all_sage_data
       WHERE property_id = 'ee1678f6-7a81-4e70-945e-bd24186a9996'
       ORDER BY
         CASE unit_type
           WHEN 'Cabin' THEN 1
           WHEN 'Tipi' THEN 2
    ...
    console.log('✓ Taos Goji enrichment applied');
    console.table(rows);
