#!/usr/bin/env npx tsx
/**
 * Research glamping resorts in United Kingdom using OpenAI API and save to CSV
 * 
 * This script:
 * - Uses OpenAI API to research glamping resorts in United Kingdom
 ...
 * 
 * Usage:
 *   npx tsx scripts/research-uk-glamping-resorts.ts
 */
...
const CSV_FILE = 'csv/glamping-properties/europe/uk-glamping-resorts.csv';

// Search queries to find glamping resorts in United Kingdom
const SEARCH_QUERIES = [
  'best glamping resorts UK 2024',
  'luxury glamping United Kingdom',
  'glamping sites Cornwall UK',
  'glamping sites Lake District UK',
  'glamping sites Cotswolds UK',
  'glamping sites Scotland UK',
  'glamping sites Wales UK',
  'glamping sites Yorkshire UK',
  'glamping sites Devon UK',
  'glamping sites Norfolk UK',
  'treehouse glamping UK',
  'yurt glamping UK',
  'safari tent glamping UK',
  'luxury camping UK',
  'glamping retreats UK',
  'eco glamping UK',
  'glamping near London UK',
  'glamping near Edinburgh UK',
];
...
/**
 * Research glamping properties in United Kingdom using OpenAI
 */
async function researchGlampingProperties(searchQuery: string): Promise<PropertyData[]> {
  console.log(`🔍 Researching: "${searchQuery}"\n`);

  const prompt = `Research glamping resorts and retreats in United Kingdom based on this search query: "${searchQuery}"

Find glamping properties (luxury camping resorts, tent accommodations, yurts, treehouses, safari tents, etc.) located in United Kingdom (England, Scotland, Wales, or Northern Ireland).
...
- state (optional): County/region name (e.g., "Cornwall", "Cumbria", "Devon", "Highlands", "Gwynedd")
...
- phone_number (optional): Phone number if available (with country code +44)
  ...
  try {
    ...
    properties = properties
      ...
      .map(p => ({
        ...
        country: p.country?.trim() || 'United Kingdom',
      }));
...
async function enrichPropertyData(property: PropertyData): Promise<PropertyData> {
  ...
  const prompt = `Research and provide comprehensive detailed information about this glamping property in United Kingdom:
...
Return a JSON object with these fields:
{
  ...
  "city": "City name",
  "state": "County/region name (e.g., Cornwall, Cumbria, Devon, Highlands, Gwynedd)",
  "country": "United Kingdom",
  "address": "Full street address if available",
  "zip_code": "Postal code if available",
  "url": "Official website URL",
  "phone_number": "Phone number with country code (+44)",
...
Focus on UK glamping properties. If you cannot find specific information, use null or omit the field. Be thorough and accurate.
...
async function main() {
  ...
  console.log('Research Glamping Resorts in United Kingdom');
  console.log('='.repeat(70));
