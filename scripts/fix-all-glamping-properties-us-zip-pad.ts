/**
 * Left-pad US ZIP codes in all_glamping_properties that are 3 or 4 digits (leading zero lost).
 *
 * Rules (mirror scripts/migrations/fix-all-glamping-properties-us-zip-leading-zero.sql):
 * - Pad trim(zip_code) to length 5 with leading zeros when all digits and length 3 or 4
 * - Only when country is US (or null with a US state/territory abbreviation or full name)
 ...
 *
 * Usage:
 *   npx tsx scripts/fix-all-glamping-properties-us-zip-pad.ts
 *   npx tsx scripts/fix-all-glamping-properties-us-zip-pad.ts --dry-run
 */
