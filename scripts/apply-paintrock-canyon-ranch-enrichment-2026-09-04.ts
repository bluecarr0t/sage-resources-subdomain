#!/usr/bin/env npx tsx
/**
 * Publish Paintrock Canyon Ranch tent + campus inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-paintrock-canyon-ranch-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-paintrock-canyon-ranch-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              google_place_id, year_site_opened
       FROM all_sage_data
       WHERE property_id = 'c1b82baa-0cf1-48e3-b7b9-44ab0c5e46c1'
       ORDER BY id`
    );
    console.log('✓ Paintrock Canyon Ranch enrichment applied');
    console.table(rows);
