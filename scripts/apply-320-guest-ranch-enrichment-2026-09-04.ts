#!/usr/bin/env npx tsx
/**
 * Publish 320 Guest Ranch cabin inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-320-guest-ranch-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-320-guest-ranch-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              unit_pets, property_pool, property_hot_tub, property_restaurant,
              rate_basis, rate_avg_retail_daily_rate, rate_summer_weekday,
              year_site_opened
       FROM all_sage_data
       WHERE property_id = 'd96fbbdd-5988-4fa5-8258-ffe6eee2940e'
       ORDER BY
         CASE site_name
           WHEN 'Deluxe Cabin' THEN 1
           WHEN 'Riverfront Cabin' THEN 2
           WHEN 'Luxury Log Home' THEN 3
           WHEN 'Mountain Chalet' THEN 4
           WHEN 'Historic McGill Cabin' THEN 5
           ELSE 6
         END,
         id`
    );
    console.log('✓ 320 Guest Ranch enrichment applied');
    console.table(rows);
