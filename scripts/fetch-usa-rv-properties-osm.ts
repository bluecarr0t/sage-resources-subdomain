#!/usr/bin/env npx tsx
/**
 * Fetch RV Parks and RV Resorts from all USA states using OpenStreetMap Overpass API
 * 
 * This script:
 * - Queries OpenStreetMap Overpass API for all campgrounds (tourism=camp_site) in all 50 US states
 * - Filters for properties with "RV Park" or "RV Resort" in the name
 * - Extracts campground data including RV-specific features
 * - Stores in osm_rv_properties table
 * 
 * Usage:
...
// All 50 US states with bounding boxes - large states split into regions
const STATE_REGIONS: StateRegion[] = [
  // California - split into 3 regions
  ...
  // Washington - split into 2 regions
  ...
  // Arizona - split into 2 regions
  { name: 'Northern Arizona', state: 'AZ', south: 34.0, west: -114.8, north: 37.0, east: -109.0 },
  { name: 'Southern Arizona', state: 'AZ', south: 31.3, west: -114.8, north: 34.0, east: -109.0 },
  // New Mexico - split into 2 regions
  { name: 'Northern New Mexico', state: 'NM', south: 34.0, west: -109.0, north: 37.0, east: -103.0 },
  { name: 'Southern New Mexico', state: 'NM', south: 31.3, west: -109.0, north: 34.0, east: -103.0 },
  // Montana - split into 2 regions
  { name: 'Western Montana', state: 'MT', south: 44.4, west: -116.1, north: 49.0, east: -110.0 },
  { name: 'Eastern Montana', state: 'MT', south: 44.4, west: -110.0, north: 49.0, east: -104.0 },
  // Wyoming
  { name: 'Wyoming', state: 'WY', south: 41.0, west: -111.1, north: 45.0, east: -104.1 },
  // Utah - split into 2 regions
  { name: 'Northern Utah', state: 'UT', south: 39.0, west: -114.1, north: 42.0, east: -109.0 },
  { name: 'Southern Utah', state: 'UT', south: 37.0, west: -114.1, north: 39.0, east: -109.0 },
  // Colorado - split into 2 regions
  { name: 'Western Colorado', state: 'CO', south: 37.0, west: -109.1, north: 41.0, east: -105.5 },
  { name: 'Eastern Colorado', state: 'CO', south: 37.0, west: -105.5, north: 41.0, east: -102.0 },
  // Texas - split into 4 regions (large state)
  ...
  { name: 'Eastern Texas', state: 'TX', south: 25.8, west: -96.0, north: 32.0, east: -93.5 },
  ...
  // Georgia - split into 2 regions
  { name: 'Northern Georgia', state: 'GA', south: 32.5, west: -85.6, north: 35.0, east: -80.8 },
  { name: 'Southern Georgia', state: 'GA', south: 30.4, west: -85.6, north: 32.5, east: -80.8 },
  // Florida - split into 3 regions
  { name: 'Northern Florida', state: 'FL', south: 29.0, west: -87.6, north: 31.0, east: -80.0 },
  { name: 'Central Florida', state: 'FL', south: 27.0, west: -82.7, north: 29.0, east: -80.0 },
  { name: 'Southern Florida', state: 'FL', south: 24.5, west: -81.8, north: 27.0, east: -79.8 },
  // South Carolina
  ...
  // Pennsylvania - split into 2 regions
  { name: 'Western Pennsylvania', state: 'PA', south: 39.7, west: -80.5, north: 42.3, east: -77.5 },
  { name: 'Eastern Pennsylvania', state: 'PA', south: 39.7, west: -77.5, north: 42.3, east: -74.7 },
  // New York - split into 2 regions
  { name: 'Western New York', state: 'NY', south: 40.5, west: -79.8, north: 45.0, east: -76.0 },
  { name: 'Eastern New York', state: 'NY', south: 40.5, west: -76.0, north: 45.0, east: -71.8 },
  // Connecticut
  ...
  // Michigan - split into 2 regions
  { name: 'Lower Peninsula Michigan', state: 'MI', south: 41.7, west: -87.2, north: 46.0, east: -82.1 },
  { name: 'Upper Peninsula Michigan', state: 'MI', south: 45.0, west: -90.4, north: 48.3, east: -83.5 },
  // Wisconsin
  { name: 'Wisconsin', state: 'WI', south: 42.5, west: -92.9, north: 47.1, east: -86.2 },
  // Alaska - split into 3 regions (very large state)
  { name: 'Southeast Alaska', state: 'AK', south: 54.0, west: -140.0, north: 61.0, east: -130.0 },
  { name: 'Southcentral Alaska', state: 'AK', south: 59.0, west: -154.0, north: 63.0, east: -140.0 },
  { name: 'Interior Alaska', state: 'AK', south: 63.0, west: -165.0, north: 71.4, east: -141.0 },
  // Hawaii - split into 2 regions
  { name: 'Hawaii Main Islands', state: 'HI', south: 18.9, west: -160.3, north: 22.3, east: -154.8 },
  { name: 'Hawaii Big Island', state: 'HI', south: 18.9, west: -156.1, north: 20.3, east: -154.8 },
];
...
interface ProcessingStats {
  ...
  skipped: number;
  failed: number;
  rvFiltered: number; // Campgrounds filtered out for not matching RV Park/Resort criteria
}
 ...
 */
function generateSlug(name: string, city: string | null, state: string): string {
  ...
  baseSlug = `${baseSlug}-${state.toLowerCase()}`;
...
async function insertRVProperty(
  ...
  const slug = generateSlug(extractedData.name, extractedData.city, state);
...
async function main() {
  ...
  console.log('🚀 Starting USA RV Parks and Resorts Discovery from OpenStreetMap');
  console.log('='.repeat(80));
  console.log();
  console.log('📋 Scope: All 50 US States');
  ...
  try {
    ...
    if (stats.inserted > 0) {
      console.log(`\n✅ Successfully discovered and inserted ${stats.inserted} RV Parks/Resorts!`);
    }
