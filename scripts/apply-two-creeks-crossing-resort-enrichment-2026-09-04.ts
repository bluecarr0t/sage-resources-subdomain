#!/usr/bin/env npx tsx
/**
 * Publish Two Creeks Crossing Resort and split RV / cabin / yurt / treehouse / tent inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-two-creeks-crossing-resort-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-two-creeks-crossing-resort-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              unit_pets, property_pool, property_waterpark, rate_basis,
              rate_avg_retail_daily_rate, rate_summer_weekday
       FROM all_sage_data
       WHERE property_id = 'd6a39f3c-39c5-496e-8a5d-91c61ee6b76d'
       ORDER BY
         CASE unit_type
           WHEN 'RV Site' THEN 1
           WHEN 'Cabin' THEN 2
           WHEN 'Yurt' THEN 3
           WHEN 'Treehouse' THEN 4
           WHEN 'Safari Tent' THEN 5
           ELSE 6
         END,
         site_name`
    );
    console.log('✓ Two Creeks Crossing Resort enrichment applied');
    console.table(rows);
