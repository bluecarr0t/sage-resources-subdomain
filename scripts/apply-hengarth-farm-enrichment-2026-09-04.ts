#!/usr/bin/env npx tsx
/**
 * Publish Hengarth Farm bell-tent inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-hengarth-farm-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-hengarth-farm-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
       ...
       FROM all_sage_data
       WHERE property_id = '4a4e672e-411c-4c3e-a303-d242663955f4'
       ORDER BY id`
    );
    console.log('✓ Hengarth Farm enrichment applied');
    console.table(rows);
