#!/usr/bin/env npx tsx
/**
 * Publish Acorns Resort cabin / lodge / yurt / RV inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-acorns-resort-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-acorns-resort-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
       ...
       FROM all_sage_data
       WHERE property_id = '1893a0c3-1cbc-4e6e-bf02-a6d6f96df3b2'
       ORDER BY id`
    );
    console.log('✓ Acorns Resort enrichment applied');
    console.table(rows);
