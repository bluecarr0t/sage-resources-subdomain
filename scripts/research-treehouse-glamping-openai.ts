#!/usr/bin/env npx tsx
/**
 * Research glamping properties with TREEHOUSE unit type, minimum 5 total units.
 * North America (USA, Canada, Mexico). Fuzzy dedup on property_name, address, city, state.
 * Inserts with research_status = 'new'.
 *
 * Usage:
 *   npx tsx scripts/research-treehouse-glamping-openai.ts
 *   npx tsx scripts/research-treehouse-glamping-openai.ts --dry-run
 *   npx tsx scripts/research-treehouse-glamping-openai.ts --limit 10
 */
...
function hasMinimumUnits(p: DiscoveredProperty): boolean {
  ...
  if (units == null) return true; // Include if unknown (let user review)
...
function buildResearchQueries(limit?: number): ResearchQuery[] {
  ...
  const unitRequirement = `CRITICAL: Include ONLY properties that have at least ${MIN_UNITS} treehouse units (quantity_of_units or property_total_sites >= ${MIN_UNITS}). Exclude single-treehouse or 1-4 unit properties.`;

  return [
    {
      label: 'USA - Treehouse Resorts (5+ units)',
      country: 'USA',
      prompt: `You are a researcher for an outdoor hospitality directory. List TREEHOUSE glamping resorts in the United States.

${unitRequirement}

Include properties that:
- Offer treehouse accommodations as a primary unit type (treehouses, tree houses)
- Have at least 5 treehouse units on site
- Are currently operating
- May also offer cabins or other units, but treehouses must be a main offering

${limitClause}

For each property provide: property_name (required), city (required), state (required), address, zip_code, url (official website), phone_number, description (2-4 sentences emphasizing treehouse experience), property_type (e.g. "Treehouse Resort", "Glamping Resort"), unit_type (must include "treehouse" or "treehouses"), quantity_of_units (required - number of treehouse units, must be >= 5), property_total_sites (if different), lat, lon if known.

Return a JSON object with a "properties" array. Only include properties with 5+ treehouse units.`,
    },
    {
      label: 'Canada - Treehouse Resorts (5+ units)',
      country: 'Canada',
      prompt: `You are a researcher for an outdoor hospitality directory. List TREEHOUSE glamping resorts in Canada.

${unitRequirement}

Include properties across provinces (BC, Ontario, Quebec, etc.) that:
- Offer treehouse accommodations as a primary unit type
- Have at least 5 treehouse units on site
- Are currently operating

${limitClause}

For each property provide: property_name (required), city (required), state (province), address, zip_code (postal code), url (official website), phone_number, description (2-4 sentences), property_type, unit_type (must include "treehouse"), quantity_of_units (required, >= 5), property_total_sites, lat, lon if known. Use country: "Canada".

Return a JSON object with a "properties" array. Only include properties with 5+ treehouse units.`,
    },
    {
      label: 'Mexico - Treehouse Resorts (5+ units)',
      country: 'Mexico',
      prompt: `You are a researcher for an outdoor hospitality directory. List TREEHOUSE glamping resorts in Mexico.

${unitRequirement}

Include properties in Tulum, Riviera Maya, Oaxaca, and other regions that:
- Offer treehouse accommodations (casas en los árboles, treehouses)
- Have at least 5 treehouse units on site
- Are currently operating

${limitClause}

For each property provide: property_name (required), city (required), state (region), address, zip_code, url (official website), phone_number, description (2-4 sentences), property_type, unit_type (must include "treehouse"), quantity_of_units (required, >= 5), property_total_sites, lat, lon if known. Use country: "Mexico".
...
async function main() {
  ...
  console.log(`Researching TREEHOUSE properties (min ${MIN_UNITS} units) in North America...\n`);
  ...
  console.log(`\nTotal unique new treehouse properties to add: ${uniqueNew.length}`);
  ...
  console.log(`\nDone. Added ${uniqueNew.length} treehouse properties (min ${MIN_UNITS} units) with research_status='new'.`);
}
