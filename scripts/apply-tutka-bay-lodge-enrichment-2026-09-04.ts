#!/usr/bin/env npx tsx
/**
 * Publish Tutka Bay Lodge cabin inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-tutka-bay-lodge-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-tutka-bay-lodge-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              year_site_opened
       FROM all_sage_data
       WHERE property_id = '43a55271-077b-4744-bdfc-283f7a51d092'
       ORDER BY id`
    );
    console.log('✓ Tutka Bay Lodge enrichment applied');
    console.table(rows);
