#!/usr/bin/env npx tsx
/**
 * Publish Dawn Ranch and split cabin / orchard glamping inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-dawn-ranch-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(process.cwd(), 'scripts/migrations/enrich-dawn-ranch-2026-09-04.sql'),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              unit_pets, property_pool, property_restaurant, property_sauna,
              rate_basis, rate_avg_retail_daily_rate, rate_fall_weekday
       FROM all_sage_data
       WHERE property_id = 'e8e5f2e8-da54-42e6-8266-1155e37b5748'
       ORDER BY
         CASE unit_type
           WHEN 'Cabin' THEN 1
           WHEN 'Safari Tent' THEN 2
           ELSE 3
         END,
         site_name`
    );
    console.log('✓ Dawn Ranch enrichment applied');
    console.table(rows);
