#!/usr/bin/env npx tsx
/**
 * Research glamping resorts in Spain using OpenAI API and save to CSV
 * 
 * This script:
 * - Uses OpenAI API to research glamping resorts in Spain
 ...
 * 
 * Usage:
 *   npx tsx scripts/research-spain-glamping-resorts.ts
 */
...
const CSV_FILE = 'csv/glamping-properties/europe/spain-glamping-resorts.csv';

// Search queries to find glamping resorts in Spain
const SEARCH_QUERIES = [
  'best glamping resorts Spain 2024',
  'luxury glamping España',
  'glamping sites Costa Brava Spain',
  'glamping sites Costa del Sol Spain',
  'glamping sites Andalusia Spain',
  'glamping sites Catalonia Spain',
  'glamping sites Balearic Islands Spain',
  'glamping sites Canary Islands Spain',
  'glamping sites Pyrenees Spain',
  'glamping sites Sierra Nevada Spain',
  'treehouse glamping Spain',
  'yurt glamping Spain',
  'safari tent glamping Spain',
  'luxury camping Spain',
  'glamping retreats Spain',
  'eco glamping Spain',
  'glamping near Barcelona Spain',
  'glamping near Madrid Spain',
];
...
/**
 * Research glamping properties in Spain using OpenAI
 */
async function researchGlampingProperties(searchQuery: string): Promise<PropertyData[]> {
  console.log(`🔍 Researching: "${searchQuery}"\n`);

  const prompt = `Research glamping resorts and retreats in Spain based on this search query: "${searchQuery}"

Find glamping properties (luxury camping resorts, tent accommodations, yurts, treehouses, safari tents, etc.) located in Spain.
...
- state (optional): Spanish autonomous community/region name (e.g., "Andalusia", "Catalonia", "Balearic Islands")
- country: "Spain"
- url (optional): Official website URL if you can find it
- description (optional): Brief description of what makes it special
- unit_type (optional): Types of accommodations (e.g., "tents", "yurts", "treehouses", "safari tents", "cabins")
...
- phone_number (optional): Phone number if available (with country code +34)
...
async function enrichPropertyData(property: PropertyData): Promise<PropertyData> {
  ...
  const prompt = `Research and provide comprehensive detailed information about this glamping property in Spain:
...
Return a JSON object with these fields:
{
  ...
  "city": "City name",
  "state": "Spanish autonomous community/region name (e.g., Andalusia, Catalonia, Balearic Islands)",
  "country": "Spain",
  "address": "Full street address if available",
  "zip_code": "Postal code if available",
  "url": "Official website URL",
  "phone_number": "Phone number with country code (+34)",
  ...
  "unit_type": "Types of accommodations (comma-separated, e.g., 'safari tents, yurts, treehouses')",
...
Focus on Spanish glamping properties. If you cannot find specific information, use null or omit the field. Be thorough and accurate.
...
async function main() {
  ...
  console.log('Research Glamping Resorts in Spain');
  console.log('='.repeat(70));
