#!/usr/bin/env npx tsx
/**
 * Research glamping properties with YURT unit type, minimum 5 total units.
 * North America (USA, Canada, Mexico). Fuzzy dedup on property_name, address, city, state.
 * Inserts with research_status = 'new'.
 *
 * Usage:
 *   npx tsx scripts/research-yurt-glamping-openai.ts
 *   npx tsx scripts/research-yurt-glamping-openai.ts --dry-run
 *   npx tsx scripts/research-yurt-glamping-openai.ts --limit 10
 */
...
function buildResearchQueries(limit?: number): ResearchQuery[] {
  ...
  const unitRequirement = `CRITICAL: Include ONLY properties that have at least ${MIN_UNITS} yurt units (quantity_of_units or property_total_sites >= ${MIN_UNITS}). Exclude single-yurt or 1-4 unit properties.`;

  return [
    {
      label: 'USA - Yurt Resorts (5+ units)',
      country: 'USA',
      prompt: `You are a researcher for an outdoor hospitality directory. List YURT glamping resorts in the United States.

${unitRequirement}

Include properties that:
- Offer yurts as a primary unit type (yurts, Mongolian-style round tents)
- Have at least 5 yurt units on site
- Are currently operating
- May also offer cabins or other units, but yurts must be a main offering
- Exclude traditional campgrounds with only basic tents

${limitClause}

For each property provide: property_name (required), city (required), state (required), address, zip_code, url (official website), phone_number, description (2-4 sentences emphasizing yurt experience), property_type (e.g. "Glamping Resort", "Yurt Resort"), unit_type (must include "yurt" or "yurts"), quantity_of_units (required - number of yurt units, must be >= 5), property_total_sites (if different), lat, lon if known.

Return a JSON object with a "properties" array. Only include properties with 5+ yurt units.`,
    },
    {
      label: 'Canada - Yurt Resorts (5+ units)',
      country: 'Canada',
      prompt: `You are a researcher for an outdoor hospitality directory. List YURT glamping resorts in Canada.

${unitRequirement}

Include properties across provinces (BC, Ontario, Quebec, etc.) that:
- Offer yurts as a primary unit type
- Have at least 5 yurt units on site
- Are currently operating

${limitClause}

For each property provide: property_name (required), city (required), state (province), address, zip_code (postal code), url (official website), phone_number, description (2-4 sentences), property_type, unit_type (must include "yurt"), quantity_of_units (required, >= 5), property_total_sites, lat, lon if known. Use country: "Canada".

Return a JSON object with a "properties" array. Only include properties with 5+ yurt units.`,
    },
    {
      label: 'Mexico - Yurt Resorts (5+ units)',
      country: 'Mexico',
      prompt: `You are a researcher for an outdoor hospitality directory. List YURT glamping resorts in Mexico.

${unitRequirement}

Include properties in Tulum, Riviera Maya, Oaxaca, and other regions that:
- Offer yurts as a primary unit type (yurtas, glamping yurts)
- Have at least 5 yurt units on site
- Are currently operating

${limitClause}

For each property provide: property_name (required), city (required), state (region), address, zip_code, url (official website), phone_number, description (2-4 sentences), property_type, unit_type (must include "yurt"), quantity_of_units (required, >= 5), property_total_sites, lat, lon if known. Use country: "Mexico".
...
function toDbRow(p: DiscoveredProperty): Record<string, unknown> {
  ...
  return {
    ...
    is_closed: 'No',
    source: 'Sage',
    discovery_source: 'OpenAI Research - Yurts',
    date_added: TODAY,
    date_updated: TODAY,
...
async function main() {
  ...
  console.log(`Researching YURT properties (min ${MIN_UNITS} units) in North America...\n`);
  ...
  console.log(`\nTotal unique new yurt properties to add: ${uniqueNew.length}`);
  ...
  console.log(`\nDone. Added ${uniqueNew.length} yurt properties (min ${MIN_UNITS} units) with research_status='new'.`);
}
