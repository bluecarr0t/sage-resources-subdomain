#!/usr/bin/env npx tsx
/**
 * Research glamping resorts in Netherlands using OpenAI API and save to CSV
 * 
 * This script:
 * - Uses OpenAI API to research glamping resorts in Netherlands
 ...
 * 
 * Usage:
 *   npx tsx scripts/research-netherlands-glamping-resorts.ts
 */
...
const CSV_FILE = 'csv/glamping-properties/europe/netherlands-glamping-resorts.csv';

// Search queries to find glamping resorts in Netherlands
const SEARCH_QUERIES = [
  'best glamping resorts Netherlands 2024',
  'luxury glamping Nederland',
  'glamping sites Amsterdam Netherlands',
  'glamping sites Zeeland Netherlands',
  'glamping sites Friesland Netherlands',
  'glamping sites Gelderland Netherlands',
  'glamping sites North Holland Netherlands',
  'glamping sites South Holland Netherlands',
  'glamping sites Limburg Netherlands',
  'treehouse glamping Netherlands',
  'yurt glamping Netherlands',
  'safari tent glamping Netherlands',
  'luxury camping Netherlands',
  'glamping retreats Netherlands',
  'eco glamping Netherlands',
  'glamping near Amsterdam Netherlands',
  'glamping near Rotterdam Netherlands',
  'glamping near Utrecht Netherlands',
];
...
/**
 * Research glamping properties in Netherlands using OpenAI
 */
async function researchGlampingProperties(searchQuery: string): Promise<PropertyData[]> {
  console.log(`🔍 Researching: "${searchQuery}"\n`);

  const prompt = `Research glamping resorts and retreats in Netherlands based on this search query: "${searchQuery}"

Find glamping properties (luxury camping resorts, tent accommodations, yurts, treehouses, safari tents, etc.) located in Netherlands.
...
- state (optional): Dutch province name (e.g., "North Holland", "South Holland", "Gelderland", "Friesland", "Zeeland")
- country: "Netherlands"
...
- phone_number (optional): Phone number if available (with country code +31)
  ...
  try {
    ...
    properties = properties
      ...
      .map(p => ({
        ...
        country: p.country?.trim() || 'Netherlands',
      }));
...
async function enrichPropertyData(property: PropertyData): Promise<PropertyData> {
  ...
  const prompt = `Research and provide comprehensive detailed information about this glamping property in Netherlands:
...
Return a JSON object with these fields:
{
  ...
  "city": "City name",
  "state": "Dutch province name (e.g., North Holland, South Holland, Gelderland, Friesland, Zeeland)",
  "country": "Netherlands",
  "address": "Full street address if available",
  "zip_code": "Postal code if available (format: 1234 AB)",
  "url": "Official website URL",
  "phone_number": "Phone number with country code (+31)",
  ...
  "operating_season__months_": "Operating months (e.g., 'April-October' or 'Year-round')",
...
Focus on Dutch glamping properties. If you cannot find specific information, use null or omit the field. Be thorough and accurate.
...
async function main() {
  ...
  console.log('Research Glamping Resorts in Netherlands');
  console.log('='.repeat(70));
