#!/usr/bin/env npx tsx
/**
 * Publish Noble Pine Campground A-frame / tent / cabin / RV inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-noble-pine-campground-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-noble-pine-campground-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
       ...
       FROM all_sage_data
       WHERE property_id = 'fbe6a571-ecb2-4010-bf25-4f41445f1f7d'
       ORDER BY id`
    );
    console.log('✓ Noble Pine Campground enrichment applied');
    console.table(rows);
