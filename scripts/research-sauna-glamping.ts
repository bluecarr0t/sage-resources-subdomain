#!/usr/bin/env npx tsx
/**
 * Web research sauna tagging for published US glamping cohort.
 *
 * Usage:
 *   npx tsx scripts/research-sauna-glamping.ts --dry-run --limit 25
 *   npx tsx scripts/research-sauna-glamping.ts --limit 50 --only-null
 *   npx tsx scripts/research-sauna-glamping.ts --property-id <uuid>
 *   npx tsx scripts/research-sauna-glamping.ts --export-sql --limit 100
 *   npx tsx scripts/research-sauna-glamping.ts --report
 *
...
import {
  ...
  pickScrapeUrl,
  SAUNA_TABLE,
} from '@/lib/glamping-sauna-research/cohort';
import { extractSaunaFromMarkdown } from '@/lib/glamping-sauna-research/extract';
import {
  buildApplyResult,
  applyResultToSqlUpdates,
} from '@/lib/glamping-sauna-research/apply';
import type {
  PropertyResearchResult,
  SaunaConflict,
} from '@/lib/glamping-sauna-research/types';
import { todayIsoDate } from '@/lib/glamping-sauna-research/normalize';
...
const OUT_DIR = resolve(process.cwd(), 'scripts/.tmp-sauna-review');
...
function appendConflictCsv(conflicts: SaunaConflict[]) {
...
function runReport() {
  console.log('Run queries/sauna_tagging_audit.sql in Supabase for coverage metrics.');
  console.log(`Review artifacts: ${OUT_DIR}`);
}

async function processProperty(
  propertyId: string,
  rows: import('@/lib/glamping-sauna-research/types').SaunaCohortRow[],
  ...
  const extraction = await extractSaunaFromMarkdown(
    openai,
    rows,
    scraped.markdown,
    scraped.sources
...
async function main() {
  ...
  if (opts.exportSql && sqlChunks.length > 0) {
    const sqlPath = join(OUT_DIR, `backfill-sauna-${todayIsoDate()}.sql`);
