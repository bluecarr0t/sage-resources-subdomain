-- Paginated property-level list for /admin/comps (one row per source + address_key).
...
BEGIN
  ...
  ORDER BY
    ...
    CASE WHEN v_sort = 'low_adr' AND NOT p_sort_asc THEN (g.anchor->>'low_adr') END DESC NULLS LAST,
...
BEGIN
  ...
  WHERE n.nspname = 'public'
    AND p.proname = 'unified_comps_list_properties'
  LIMIT 1;
