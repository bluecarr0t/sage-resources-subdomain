...
--   2. all_sage_data                                     (source='all_sage_data')
...
-- Sage `all_sage_data` is unit-centric: the accommodation label often
...
SELECT
  ...
  -- Sage: `all_sage_data.url` only (no `google_website_uri` column in this schema).
...
CREATE INDEX unified_comps_sage_is_open
  ...
  WHERE source = 'all_sage_data' AND is_open IS NOT NULL;
