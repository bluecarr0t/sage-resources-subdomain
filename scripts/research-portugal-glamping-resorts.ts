#!/usr/bin/env npx tsx
/**
 * Research glamping resorts in Portugal using OpenAI API and save to CSV
 * 
 * This script:
 * - Uses OpenAI API to research glamping resorts in Portugal
 ...
 * 
 * Usage:
 *   npx tsx scripts/research-portugal-glamping-resorts.ts
 */
...
const CSV_FILE = 'csv/glamping-properties/europe/portugal-glamping-resorts.csv';

// Search queries to find glamping resorts in Portugal
const SEARCH_QUERIES = [
  'best glamping resorts Portugal 2024',
  'luxury glamping Portugal',
  'glamping sites Algarve Portugal',
  'glamping sites Lisbon Portugal',
  'glamping sites Porto Portugal',
  'glamping sites Alentejo Portugal',
  'glamping sites Douro Valley Portugal',
  'glamping sites Sintra Portugal',
  'glamping sites Madeira Portugal',
  'glamping sites Azores Portugal',
  'treehouse glamping Portugal',
  'yurt glamping Portugal',
  'safari tent glamping Portugal',
  'luxury camping Portugal',
  'glamping retreats Portugal',
  'eco glamping Portugal',
  'glamping near Lisbon Portugal',
  'glamping near Porto Portugal',
];
...
/**
 * Research glamping properties in Portugal using OpenAI
 */
async function researchGlampingProperties(searchQuery: string): Promise<PropertyData[]> {
  console.log(`🔍 Researching: "${searchQuery}"\n`);

  const prompt = `Research glamping resorts and retreats in Portugal based on this search query: "${searchQuery}"

Find glamping properties (luxury camping resorts, tent accommodations, yurts, treehouses, safari tents, etc.) located in Portugal.
...
- state (optional): Portuguese region/district name (e.g., "Algarve", "Lisbon", "Porto", "Alentejo", "Madeira", "Azores")
- country: "Portugal"
...
- phone_number (optional): Phone number if available (with country code +351)
  ...
  try {
    ...
    properties = properties
      ...
      .map(p => ({
        ...
        country: p.country?.trim() || 'Portugal',
      }));
...
async function enrichPropertyData(property: PropertyData): Promise<PropertyData> {
  ...
  const prompt = `Research and provide comprehensive detailed information about this glamping property in Portugal:
...
Return a JSON object with these fields:
{
  ...
  "city": "City name",
  "state": "Portuguese region/district name (e.g., Algarve, Lisbon, Porto, Alentejo, Madeira, Azores)",
  "country": "Portugal",
  "address": "Full street address if available",
  "zip_code": "Postal code if available",
  "url": "Official website URL",
  "phone_number": "Phone number with country code (+351)",
...
Focus on Portuguese glamping properties. If you cannot find specific information, use null or omit the field. Be thorough and accurate.
...
async function main() {
  ...
  console.log('Research Glamping Resorts in Portugal');
  console.log('='.repeat(70));
