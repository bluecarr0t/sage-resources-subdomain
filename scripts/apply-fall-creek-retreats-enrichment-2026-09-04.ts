#!/usr/bin/env npx tsx
/**
 * Publish Fall Creek Retreats A-frame / geodome / tiny-home / cabin inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-fall-creek-retreats-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-fall-creek-retreats-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
       ...
       FROM all_sage_data
       WHERE property_id = '87acd71c-d907-4dfe-b2a9-f6ba31a9b142'
       ORDER BY id`
    );
    console.log('✓ Fall Creek Retreats enrichment applied');
    console.table(rows);
