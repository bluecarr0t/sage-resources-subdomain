#!/usr/bin/env npx tsx
/**
 * Research glamping resorts in France using OpenAI API and save to CSV
 * 
 * This script:
 * - Uses OpenAI API to research glamping resorts in France
 ...
 * 
 * Usage:
 *   npx tsx scripts/research-france-glamping-resorts.ts
 */
...
const CSV_FILE = 'csv/glamping-properties/europe/france-glamping-resorts.csv';

// Search queries to find glamping resorts in France
const SEARCH_QUERIES = [
  'best glamping resorts France 2024',
  'luxury glamping France',
  'glamping sites Provence France',
  'glamping sites French Riviera France',
  'glamping sites Loire Valley France',
  'glamping sites Dordogne France',
  'glamping sites Normandy France',
  'glamping sites Brittany France',
  'glamping sites French Alps France',
  'glamping sites Pyrenees France',
  'treehouse glamping France',
  'yurt glamping France',
  'safari tent glamping France',
  'luxury camping France',
  'glamping retreats France',
  'eco glamping France',
  'glamping near Paris France',
  'glamping near Nice France',
];
...
/**
 * Research glamping properties in France using OpenAI
 */
async function researchGlampingProperties(searchQuery: string): Promise<PropertyData[]> {
  console.log(`🔍 Researching: "${searchQuery}"\n`);

  const prompt = `Research glamping resorts and retreats in France based on this search query: "${searchQuery}"

Find glamping properties (luxury camping resorts, tent accommodations, yurts, treehouses, safari tents, etc.) located in France.
...
- state (optional): French region name (e.g., "Provence-Alpes-Côte d'Azur", "Nouvelle-Aquitaine", "Occitanie")
- country: "France"
...
- phone_number (optional): Phone number if available (with country code +33)
...
async function enrichPropertyData(property: PropertyData): Promise<PropertyData> {
  ...
  const prompt = `Research and provide comprehensive detailed information about this glamping property in France:
...
Return a JSON object with these fields:
{
  ...
  "city": "City name",
  "state": "French region name (e.g., Provence-Alpes-Côte d'Azur, Nouvelle-Aquitaine, Occitanie)",
  "country": "France",
  "address": "Full street address if available",
  "zip_code": "Postal code if available",
  "url": "Official website URL",
  "phone_number": "Phone number with country code (+33)",
...
Focus on French glamping properties. If you cannot find specific information, use null or omit the field. Be thorough and accurate.
...
async function main() {
  ...
  console.log('Research Glamping Resorts in France');
  console.log('='.repeat(70));
