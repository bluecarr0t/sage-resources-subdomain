/**
 * Script to upload Campspot CSV data to Supabase 'campspot' table
 *
 * Usage:
 *   npx tsx scripts/upload-campspot-csv.ts [path-to-csv]
 *
 * Default CSV path: csv/sites-Campspot-United States- (1).csv
 *
 * Prerequisites:
 *   1. Run scripts/migrations/create-campspot-table.sql in Supabase SQL Editor first
...
const TABLE_NAME = 'campspot';
const DEFAULT_CSV_PATH = 'csv/sites-Campspot-United States- (1).csv';
  ...
  try {
    ...
    if (tableError) {
      if (tableError.code === 'PGRST116' || tableError.message.includes('does not exist')) {
        console.error('\n❌ Table "campspot" does not exist.');
        console.error('Please run scripts/migrations/create-campspot-table.sql in Supabase SQL Editor first.');
        process.exit(1);
      }
      throw tableError;
    }
