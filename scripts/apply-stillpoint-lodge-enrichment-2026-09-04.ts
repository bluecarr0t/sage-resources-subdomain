#!/usr/bin/env npx tsx
/**
 * Publish Stillpoint Lodge cabin inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-stillpoint-lodge-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-stillpoint-lodge-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              unit_pets, property_hot_tub, property_restaurant, property_sauna,
              rate_basis, rate_avg_retail_daily_rate, rate_fall_weekday
       FROM all_sage_data
       WHERE property_id = 'c019237f-0bda-462d-9db1-8a591e18c96a'
       ORDER BY id`
    );
    console.log('✓ Stillpoint Lodge enrichment applied');
    console.table(rows);
