#!/usr/bin/env npx tsx
/**
 * Publish Tanque Verde Ranch, reclassify to Ranch & Lodge, set casita inventory.
 *
 * Usage:
 *   npx tsx scripts/apply-tanque-verde-ranch-enrichment-2026-09-04.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(
    process.cwd(),
    'scripts/migrations/enrich-tanque-verde-ranch-2026-09-04.sql'
  ),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              unit_pets, property_pool, property_restaurant,
              rate_basis, rate_avg_retail_daily_rate, rate_fall_weekday,
              year_site_opened, glamping_service_tier
       FROM all_sage_data
       WHERE property_id = 'f6adaaba-b7e7-4175-956a-b9ebc7e8d8a5'
       ORDER BY id`
    );
    console.log('✓ Tanque Verde Ranch enrichment applied');
    console.table(rows);
