#!/usr/bin/env npx tsx
/**
 * Research glamping resorts in Germany using OpenAI API and save to CSV
 * 
 * This script:
 * - Uses OpenAI API to research glamping resorts in Germany
 ...
 * 
 * Usage:
 *   npx tsx scripts/research-germany-glamping-resorts.ts
 */
...
const CSV_FILE = 'csv/glamping-properties/europe/germany-glamping-resorts.csv';

// Search queries to find glamping resorts in Germany
const SEARCH_QUERIES = [
  'best glamping resorts Germany 2024',
  'luxury glamping Deutschland',
  'glamping sites Bavaria Germany',
  'glamping sites Black Forest Germany',
  'glamping sites Rhine Valley Germany',
  'glamping sites Mosel Valley Germany',
  'glamping sites Baltic Sea Germany',
  'glamping sites North Sea Germany',
  'treehouse glamping Germany',
  'yurt glamping Germany',
  'safari tent glamping Germany',
  'luxury camping Germany',
  'glamping retreats Germany',
  'eco glamping Germany',
  'glamping near Berlin Germany',
  'glamping near Munich Germany',
];
...
/**
 * Research glamping properties in Germany using OpenAI
 */
async function researchGlampingProperties(searchQuery: string): Promise<PropertyData[]> {
  console.log(`🔍 Researching: "${searchQuery}"\n`);

  const prompt = `Research glamping resorts and retreats in Germany based on this search query: "${searchQuery}"

Find glamping properties (luxury camping resorts, tent accommodations, yurts, treehouses, safari tents, etc.) located in Germany.
...
- state (optional): German state (Bundesland) name
- country: "Germany"
  ...
  try {
    ...
    properties = properties
      ...
      .map(p => ({
        ...
        country: p.country?.trim() || 'Germany',
      }));
...
async function enrichPropertyData(property: PropertyData): Promise<PropertyData> {
  ...
  const prompt = `Research and provide comprehensive detailed information about this glamping property in Germany:
...
Return a JSON object with these fields:
{
  ...
  "city": "City name",
  "state": "German state (Bundesland) name",
  "country": "Germany",
  "address": "Full street address if available",
  "zip_code": "Postal code if available",
  "url": "Official website URL",
  "phone_number": "Phone number with country code (+49)",
...
Focus on German glamping properties. If you cannot find specific information, use null or omit the field. Be thorough and accurate.
...
async function main() {
  ...
  console.log('Research Glamping Resorts in Germany');
  console.log('='.repeat(70));
