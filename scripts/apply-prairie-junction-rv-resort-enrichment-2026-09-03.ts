#!/usr/bin/env npx tsx
/**
 * Publish Prairie Junction RV Resort and split into operating + Prairie Haven rows.
 *
 * Usage:
 *   npx tsx scripts/apply-prairie-junction-rv-resort-enrichment-2026-09-03.ts
 *
...
const MIGRATION_SQL = readFileSync(
  resolve(process.cwd(), 'scripts/migrations/enrich-prairie-junction-rv-resort-2026-09-03.sql'),
  'utf-8'
);
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT id, site_name, unit_type, quantity_of_units, property_total_sites,
              ...
              city, lat, lon, url, phone_number, unit_pets,
              rate_basis, rate_avg_retail_daily_rate, rate_summer_weekday,
              planned_open_date
       FROM all_sage_data
       WHERE property_id = '351aacb6-dc2e-46b3-9113-c3615b41103e'
       ORDER BY
         CASE unit_type
           WHEN 'RV Site' THEN 1
    ...
    console.log('✓ Prairie Junction RV Resort enrichment applied');
    console.table(rows);
