#!/usr/bin/env npx tsx
/**
 * Deep research: Most popular glamping resorts in North America not in all_glamping_properties.
 * Uses OpenAI API to discover properties, fuzzy dedup on property_name, address, city, state.
 * Inserts new records with research_status = 'new'.
 *
 * Usage:
 *   npx tsx scripts/research-popular-north-america-glamping-openai.ts
 *   npx tsx scripts/research-popular-north-america-glamping-openai.ts --dry-run
 *   npx tsx scripts/research-popular-north-america-glamping-openai.ts --limit 10
 */
...
function isDuplicateFuzzy(p: DiscoveredProperty, existing: ExistingRecord[]): boolean {
  ...
  for (const e of existing) {
    ...
    if (nameSim >= NAME_SIMILARITY_THRESHOLD) {
      // If property names are very similar, also check city and state
...
function buildResearchQueries(limit?: number): ResearchQuery[] {
  const limitClause = limit
    ? `Return exactly ${limit} properties (the most popular ones).`
    : 'Return 15-25 of the most popular properties.';

  return [
    {
      label: 'USA - Top Popular Glamping Resorts',
      country: 'USA',
      prompt: `You are a researcher for an outdoor hospitality directory. List the MOST POPULAR and HIGHEST-RATED glamping resorts in the United States that are NOT already in our database.

Focus on properties that:
- Are frequently featured in travel guides, "best glamping" lists, and media
- Have strong brand recognition (e.g. Under Canvas, AutoCamp, Collective Retreats, Tentrr)
- Are well-reviewed and highly rated
- Offer safari tents, domes, treehouses, luxury cabins, yurts, Airstreams, shepherd huts
- Include both luxury resorts and mid-range popular options
...
For each property provide: property_name (required), city (required), state (required), address, zip_code, url (official website), phone_number, description (2-4 sentences), property_type (e.g. "Glamping Resort", "Luxury Camping"), unit_type (single singular Title Case label preferred — e.g. "Bell Tent", "Yurt", "Dome", "Cabin", "Safari Tent", "Cabin Tent"; ambiguous "tents"/"glamping tents" → omit, not Safari Tent; avoid lowercase plurals), lat, lon if known.

Return a JSON object with a "properties" array. Be accurate - only include properties you are confident exist and are popular.`,
    },
    {
      label: 'USA - Top Landscape Hotels & Boutique Lodges',
      country: 'USA',
      prompt: `You are a researcher for an outdoor hospitality directory. List the MOST POPULAR landscape hotels and boutique nature lodges in the United States.

Landscape hotels are luxury accommodations that blend into natural surroundings with floor-to-ceiling windows, panoramic views, and architecture that integrates with nature. Include:
...
- Desert or mountain lodges with immersive nature views
- Treehouse resorts with forest views

${limitClause}

EXCLUDE properties already commonly known as "glamping" (safari tents, yurts, domes) unless they are specifically marketed as landscape hotels. Focus on design-forward, architecture-integrated stays.

For each property provide: property_name (required), city (required), state (required), address, zip_code, url (official website), phone_number, description (2-4 sentences), property_type (e.g. "Landscape Hotel", "Luxury Lodge"), unit_type, lat, lon if known.

Return a JSON object with a "properties" array. Be accurate - only include properties you are confident exist.`,
    },
    {
      label: 'Canada - Top Popular Glamping Resorts',
      country: 'Canada',
      prompt: `You are a researcher for an outdoor hospitality directory. List the MOST POPULAR glamping resorts in Canada.

Include properties across provinces: British Columbia, Alberta, Ontario, Quebec, Nova Scotia, etc.
- Safari tents, domes, treehouses, luxury cabins, yurts
- Well-reviewed and frequently featured in Canadian travel media
- Both luxury and mid-range popular options

${limitClause}

For each property provide: property_name (required), city (required), state (required - use province name or code), address, zip_code (or postal code), url (official website), phone_number, description (2-4 sentences), property_type (e.g. "Glamping Resort"), unit_type, lat, lon if known. Use country: "Canada".

Return a JSON object with a "properties" array. Be accurate - only include properties you are confident exist.`,
    },
    {
      label: 'Mexico - Top Popular Glamping Resorts',
      country: 'Mexico',
      prompt: `You are a researcher for an outdoor hospitality directory. List the MOST POPULAR glamping resorts in Mexico.

Include properties in popular regions: Tulum, Riviera Maya, Baja California, Oaxaca, etc.
- Safari tents, domes, treehouses, luxury cabins, yurts, palapas
- Well-reviewed and frequently featured in travel guides
- Both luxury and mid-range popular options

${limitClause}

For each property provide: property_name (required), city (required), state (required - use state/region name), address, zip_code, url (official website), phone_number, description (2-4 sentences), property_type (e.g. "Glamping Resort"), unit_type, lat, lon if known. Use country: "Mexico".
...
function toDbRow(p: DiscoveredProperty): Record<string, unknown> {
  ...
  return {
    ...
    is_open: 'Yes',
    source: 'Sage',
    discovery_source: 'OpenAI Research - Popular North America',
    date_added: TODAY,
    date_updated: TODAY,
...
async function main() {
  ...
  // Dedup within allNew (same property might appear for multiple queries)
  ...
  if (dryRun) {
    ...
    uniqueNew.forEach((p, i) => {
      console.log(`  ${i + 1}. ${p.property_name} (${p.city}, ${p.state}, ${p.country}) - ${p.url || 'no url'}`);
    });
    return;
  }
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    const { data, error } = await supabase.from(TABLE).insert(batch).select('id, property_name, state, country');
