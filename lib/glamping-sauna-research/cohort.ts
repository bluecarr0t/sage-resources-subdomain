...
export function isSaunaFieldEmpty(value: string | null | undefined): boolean {
  const s = String(value ?? '').trim().toLowerCase();
  return s === '' || s === 'null';
}

export function rowNeedsSaunaResearch(row: SaunaCohortRow): boolean {
  return isSaunaFieldEmpty(row.unit_sauna) && isSaunaFieldEmpty(row.property_sauna);
}

export function pickScrapeUrl(row: SaunaCohortRow): string | null {
...
export function alreadyResearchedSauna(discoverySource: string | null): boolean {
  return /web_research_sauna/i.test(String(discoverySource ?? ''));
}
...
export async function fetchCohortByProperty(
...
): Promise<Map<string, SaunaCohortRow[]>> {
  const byProperty = new Map<string, SaunaCohortRow[]>();
  ...
  for (;;) {
    ...
    const batch = (data ?? []) as SaunaCohortRow[];
    if (batch.length === 0) break;

    for (const row of batch) {
      if (isExcludedUnitType(row.unit_type)) continue;
      if (options.onlyNull && !rowNeedsSaunaResearch(row)) continue;
      if (options.skipAlreadyResearched && alreadyResearchedSauna(row.discovery_source)) {
        continue;
      }
