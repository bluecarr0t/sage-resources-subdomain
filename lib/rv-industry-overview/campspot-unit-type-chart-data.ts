 ...
 * Classification priority matches lib/comps-v2/glamping-unit-classify (vacation → RV → tent → campground → glamping).
 */
    ...
    const { data, error } = await supabase
      ...
      .select(
        'unit_type, property_name, city, state, description, quantity_of_units, ' +
          'retail_daily_rate_ytd, avg_retail_daily_rate_2025'
      )
