...
import { formatMarketLabel, MAP_MARKET_PAGES, buildMapDeepLinkPath } from '@/lib/map-market-pages';
...
export default async function MarketsIndexPage({ params }: PageProps) {
  const { locale } = params;
  if (!locales.includes(locale as Locale)) notFound();

  const t = await getTranslations({ locale, namespace: 'markets' });
  const tJourneys = await getTranslations({ locale, namespace: 'journeys' });
  ...
  return (
    ...
    >
      ...
      <p className="mt-6 text-sm font-light leading-relaxed text-neutral-600">
        <Link
          href={`/${locale}/journeys/from-comps-map-to-financed-study`}
          className={EDITORIAL_LINK_CLASS}
        >
          {tJourneys('index.title')}
        </Link>
        {' — '}
        {t('index.journeyTeaser')}
      </p>
      ...
      <section className="mt-12">
        ...
        <ul className="mt-6 divide-y divide-sage-200/70 border-y border-sage-200/70">
          {MAP_MARKET_PAGES.map((market) => (
            <li key={market.slug} className="py-5">
              <Link
              ...
              <p className="mt-2">
                <Link
                  href={buildMapDeepLinkPath(locale, market)}
                  className={EDITORIAL_LINK_CLASS}
                >
                  {t('index.openMap')}
                </Link>
