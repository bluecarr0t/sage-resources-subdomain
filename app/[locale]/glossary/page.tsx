...
export function generateStaticParams() {
  return getAvailableLocalesForContent('glossary').map((locale) => ({ locale }));
}
...
export default async function GlossaryPage({ params }: PageProps) {
  ...
  try {
    ...
    tPodcast = await getTranslations({ locale, namespace: 'podcast' });
  } catch (error) {
  ...
  return (
    ...
    >
      ...
      <EditorialCtaBand
        ...
        buttonHref={resourcesContactUsUrl('/glossary')}
        external
      />
      <PodcastSecondaryCta
        ...
        buttonHref={buildPodcastUrl({ medium: 'glossary-index' })}
      />
    </EditorialMarketingLayout>
  );
}
