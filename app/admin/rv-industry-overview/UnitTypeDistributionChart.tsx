...
import {
  ...
  type UnitTypeDistributionChartRow,
...
type Props = {
  rows: UnitTypeDistributionChartRow[];
};
...
export default function UnitTypeDistributionChart({ rows }: Props) {
  const t = useTranslations('admin.rvIndustryOverview.unitTypeDistribution');
  ...
  const yMax = useMemo(
    () => niceCeil(10, 70, ...data.map((d) => d.value)),
    [data]
  );

  const tickCount = Math.floor(yMax / 10) + 1;
  const ticks = Array.from({ length: tickCount }, (_, i) => i * 10);

  const hasAnyData = rows.some((r) => r.nProperties > 0);
