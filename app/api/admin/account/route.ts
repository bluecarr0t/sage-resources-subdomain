...
import { parsePipelineSlackPreferences } from '@/lib/project-pipeline/notifications/slack-preferences';
...
export const GET = withAdminAuth(async (_request, auth) => {
  ...
  return NextResponse.json({
    ...
    pipeline_slack_preferences: parsePipelineSlackPreferences(
      managedUser.pipeline_slack_preferences
    ),
    slack_email: managedUser.slack_email?.trim() || null,
  });
});
