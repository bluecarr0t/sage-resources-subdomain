...
import { buildCohortCsv, cohortCsvFilename } from '@/lib/market-report/cohort-csv';
...
/** National RV cohort paginates the full table — allow headroom beyond the default. */
...
const BodySchema = z
  .object({
    ...
    /** Include flattened `raw` columns (union of keys across the cohort). */
...
export const POST = withAdminAuth(async (request: NextRequest, auth) => {
  ...
  const rlKey = `market-report-cohort-csv:${userId}:${getRateLimitKey(request)}`;
  const { allowed } = await checkRateLimitAsync(rlKey, RATE_LIMIT, RATE_WINDOW_MS);
  if (!allowed) {
    return NextResponse.json(
      { success: false, code: 'RATE_LIMITED', message: 'Too many cohort CSV requests.' },
      { status: 429 }
    );
  }
  ...
  if (scope === 'local') {
    ...
    if (stateAbbr.length !== 2 && segment === 'rv_resort') {
      return NextResponse.json(
        { success: false, code: 'RV_STATE_REQUIRED' },
        { status: 400 }
      );
    }
  }
  ...
  const filename = cohortCsvFilename({
    segment,
    scope,
    adrMin: adrMin ?? null,
    adrMax: adrMax ?? null,
