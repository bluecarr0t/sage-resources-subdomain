...
## Phase 2 — Incremental upsert (implemented)

- Read-only DO pull with keyset pagination on `(updated_at, …pk)`
- `INSERT … ON CONFLICT DO UPDATE` with composite PKs from `pg_constraint`
- Auto-add missing PRIMARY KEY on Supabase (dedupe if needed)
- Weekly default includes `sites` / `propertys` (`--no-large` to skip)
- `old_data_table` skipped weekly; use `--replace-snapshots` monthly
- Commands: `npm run sync:do`, `npm run sync:do:full`, `npm test -- __tests__/scripts/sync-do-to-supabase/`
...
4. Chain downstream refresh after successful weekly run (`npm run refresh:downstream` — see [PHASE4_DOWNSTREAM_REFRESH.md](./PHASE4_DOWNSTREAM_REFRESH.md)).

## Resolved open questions

See **[DO_SUPABASE_SYNC_DECISIONS.md](./DO_SUPABASE_SYNC_DECISIONS.md)** for full detail.

| # | Question | Decision |
|---|----------|----------|
| 1 | Flat CSV source | Manual export from DO **web UI** → `upload-*-csv.ts` |
| 2 | Flat tables: full vs incremental | **Normalized mirror:** incremental. **Flat `public.*`:** full rebuild from UI CSV short-term; incremental only after Phase 3 transform is validated |
| 3 | Sync normalized schemas? | **Yes** (`hipcamp.*`, `campspot.*`, `bookoutdoors.*`) |
| 4 | Runner | **GitHub Actions** (`.github/workflows/weekly-do-sync.yml`) |
| 5 | Alerts | **Supabase only** (`do_sync_runs`, `do_sync_watermarks`) |