...
| 2 | **Centralized API auth** | Consider a shared wrapper (e.g. `withAdminAuth()`) for `/api/admin/*` to reduce risk of new routes missing auth. |
| 3 | **Input validation** | Some routes (e.g. comparables) could add stricter bounds (e.g. `page` max 1000, `per_page` max 100). |
| 4 | **RBAC** | `managed_users.role` exists but is not used for access control. Future: restrict bulk upload to admins only. |
