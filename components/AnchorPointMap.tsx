...
interface AnchorPointMapProps {
  ...
  anchorsWithCounts: Array<{ anchor_name: string; property_count_15_mi: number }>;
}

const mapContainerStyle = { width: '100%', height: '500px' };
const defaultCenter = { lat: 39.8283, lng: -98.5795 };

export function AnchorPointMap({ mapProperties, mapAnchors, anchorsWithCounts }: AnchorPointMapProps) {
  ...
  const getAnchorCount = (anchorName: string) =>
    anchorsWithCounts.find((a) => a.anchor_name === anchorName)?.property_count_15_mi ?? 0;
        ...
        {selectedProperty && (
          ...
          >
            <div className="p-2 min-w-[200px]">
              <h3 className="font-semibold text-gray-900 mb-1">{selectedProperty.property_name}</h3>
              <p className="text-sm text-gray-600 mb-1">{selectedProperty.source}</p>
          ...
          >
            <div className="p-2 min-w-[180px]">
              ...
              <p className="text-sm text-gray-600">
                {getAnchorCount(selectedAnchor.name)} properties within 15 mi
              </p>
            </div>
          </InfoWindow>
        )}
